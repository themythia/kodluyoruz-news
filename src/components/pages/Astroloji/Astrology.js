...
const Astrology = () => {
    ...
    const [txt, setTxt] = useState({ textContent: "", imageContent: "", linkContent: "", headContent: "" });
    ...
    useEffect(() => {
        if (text.length > 0) {
            ...
            let textContents = [{ textContent: [], imageContent: [], linkContent: [], headContent: [] }];
            if (feed) {
                for (let i = 0; i < 100; i++) {
                    ...
                    if (head) {
                        if (head.split('/')[0].toUpperCase().split('-').join(' ').length > 20) {
                            textContents[0].headContent.push("N LIFE");
                        } else {
                            if (head.split('/')[0].toUpperCase().split('-').join(' ') === "KULTUR VE SANAT") {
                                textContents[0].headContent.push("KÜLTÜR VE SANAT")
                            } else if (head.split('/')[0].toUpperCase().split('-').join(' ') === "MAGAZIN") {
                                textContents[0].headContent.push("MAGAZİN")
                            } else if (head.split('/')[0].toUpperCase().split('-').join(' ') === "GEZI") {
                                textContents[0].headContent.push("GEZİ")
                            } else if (head.split('/')[0].toUpperCase().split('-').join(' ') === "MODA VE GUZELLIK") {
                                textContents[0].headContent.push("MODA VE GÜZELLİK")
                            } else if (head.split('/')[0].toUpperCase().split('-').join(' ') === "SAGLIKLI YASAM") {
                                textContents[0].headContent.push("SAĞLIKLI YAŞAM")
                            } else if (head.split('/')[0].toUpperCase().split('-').join(' ') === "YASAM") {
                                textContents[0].headContent.push("YAŞAM")
                            }
                            else {
                                textContents[0].headContent.push(head.split('/')[0].toUpperCase().split('-').join(' '))
                            }
                        }
                    } else if (head2) {
                        if (head2.split('/')[0].toUpperCase().split('-').join(' ') === "DUNYA") {
                            textContents[0].headContent.push("DÜNYA")
                        } else if (head2.split('/')[0].toUpperCase().split('-').join(' ') === "YASAM") {
                            textContents[0].headContent.push("YAŞAM")
                        } else {
                            textContents[0].headContent.push(head2.split('/')[0].toUpperCase().split('-').join(' '))
                        }
                    } else if (head3) {
                        textContents[0].headContent.push(head3.split('/')[0].toUpperCase().split('-').join(' '))
    ...
    return (
        <div className='w-full mx-auto'>
            ...
            <div className='flex border-2 justify-center flex-wrap h-18 items-center '>
                <a href="https://www.ntv.com.tr/n-life/moda-ve-guzellik" className='m-5 font-bold'>
                    MODA&GÜZELLİK
                </a>
                <a href="https://www.ntv.com.tr/n-life/anne-ve-cocuk" className='m-5 font-bold'>
                    ANNE&ÇOCUK
                </a>
                <a href="https://www.ntv.com.tr/n-life/saglikli-yasam" className='m-5 font-bold'>
                    SAĞLIKLI YAŞAM
                </a>
                <a href="https://www.ntv.com.tr/n-life/kultur-ve-sanat" className='m-5 font-bold'>
                    KÜLTÜR&SANAT
                </a>
                <a href="https://www.ntv.com.tr/n-life/magazin" className='m-5 font-bold'>
                    MAGAZİN
                </a>
                <a href="https://www.ntv.com.tr/n-life/lifestyle" className='m-5 font-bold'>
                    LİFE STYLE
                </a>
                <a href="https://www.ntv.com.tr/n-life/ask-ve-iliski" className='m-5 font-bold'>
                    AŞK&İLİŞKİ
                </a>
                <a href="https://www.ntv.com.tr/n-life/n-lezzetler" className='m-5 font-bold'>
                    N-LEZZETLER
                </a>
                ...
                <div className="col-span-3 row-span-4 w-full h-full mx-auto lg:col-span-3 row-span-4 w-full h-full md:col-span-3 row-span-4 w-full h-full  sm:col-span-3 row-span-6 w-full h-full ">
                    {
                        txt[0] ? (
                            <div className='w-full h-full bg-white flex flex-col justify-center relative'>
                                ...
                                <p className='w-full absolute text-center bg-white font-bold font-sans p-2 box-border text-black lg:text-2xl bottom-45 md:text-lg bottom-25 sm:text-sm bottom-10'>{txt[0].textContent[count]}</p>
                                    ...
                                    order1.map(item => (
                                        <div className='flex flex-col justify-center items-start col-span-3 md:col-span-1 row-span-1 w-full h-full bg-white grid grid-rows-3 sm:col-span-3 mx-auto pb-3' style={{ borderBottom: '1px solid black' }} >
                                            ...
                                            <div className='row-span-1 pb-1 mt-2'>
                                                <p className='w-full h-full my-auto px-3 text-red-700 font-normal text-lg text-left uppercase' style={{ borderLeft: '3px solid rgb(185 28 28)' }}>{txt[0].headContent[item]}</p>
                ...
                <div className="col-span-3 mt-2 grid grid-cols-2 gap-4">
                    {
                        txt[0] ? (
                            <>
                                {
                                    order2.map(item => (
                                        <div className='col-span-2 row-span-2 w-full h-full relative mx-auto object-contain bg-white flex justify-center lg:col-span-1 md:col-span-1 sm:col-span-2 row-span-2 w-full h-full'>
                            ...
                            <>
                                {
                                    order3.map(item => (
                                        <div className='flex flex-col justify-center items-start col-span-3 md:col-span-1 row-span-1 w-full h-full bg-white grid grid-rows-3 sm:col-span-3 mx-auto' style={{ borderBottom: '1px solid black' }}>
                                            ...
                                            <div className='row-span-1 pb-1 mt-2'>
                                                <p className='w-full h-full px-3 text-red-700 font-normal text-lg text-left uppercase' style={{ borderLeft: '3px solid rgb(185 28 28)' }}>{txt[0].headContent[item]}</p>
                ...
                {/* kırmızı backgroundlu yer */}
                {
                    txt[0] ?
                        (<div className='col-span-3 row-span-auto mt-2 grid grid-cols-3 gap-4 bg-red-600 p-5'>
                            <span className='text-white font-bold pl-3' style={{ borderLeft: '3px solid rgb(185 28 28)', borderColor: 'white' }}>MODA & GÜZELLİK</span>
                            <span className='text-white font-bold pl-3 ' style={{ borderLeft: '3px solid rgb(185 28 28)', borderColor: 'white' }}>AŞK & İLİŞKİ</span>
                            <span className='text-white font-bold pl-3' style={{ borderLeft: '3px solid rgb(185 28 28)', borderColor: 'white' }}>N-LEZZETLER</span>
                            {
                                txt[0] ? (
                                    <>
                                        {
                            ...
                            <>
                                {
                                    order7.map(item => (
                                        <div className='flex flex-col justify-center items-start col-span-3 md:col-span-1 row-span-1 w-full h-full bg-white grid grid-rows-3 sm:col-span-3 mx-auto' style={{ borderBottom: '1px solid black' }} >
                                            <a href={`${txt[0].linkContent[item]}`} className='row-span-2 w-full h-full'>
                                                <img src={txt[0].imageContent[item]} alt='' className='row-span-2 w-full h-full bg-cover bg-center'></img>
                                            </a>
                                            <div className='row-span-1 pb-1 mt-2'>
                                                <p className='w-full h-full px-3 text-red-700 font-normal text-lg text-left uppercase' style={{ borderLeft: '3px solid rgb(185 28 28)' }}>{txt[0].headContent[item]}</p>
                                                <p className='w-full h-full  text-black font-normal text-lg text-left'>{txt[0].textContent[item]}</p>
                        ...
                        <>
                            {
                                <div className='flex bg-black justify-around h-16 items-center px-6'>
                                    <a href="/" className=''>
                                        <img className='mr-5 w-32' src="https://cdn.ntv.com.tr/img/KadinLeftLogo.svg?v=6429z" alt="ntv.com.tr"></img>
                                    </a>
                                    <p className='text-white mr-auto'>İletişim: n-life@ntv.com.tr</p>
                                    <a href="https://ntv.com.tr">
