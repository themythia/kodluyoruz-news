...
const Technology = () => {
    ...
    const style1 = 'hover:ease-in duration-300 bg-blue-900 text-white border-2 mr-2 text-base font-black px-4 py-2 rounded-full';
    const style2 = 'hover:ease-in duration-300 border-2 mr-2 text-base font-black px-4 py-2 rounded-full';
    const style3 = 'hover:ease-in duration-300 border-2 mr-2 text-base font-black px-3 py-2 rounded-full';
    const style4 = 'hover:ease-in duration-300 border-2 mr-2 text-base font-black px-4 py-2 rounded-full bg-blue-900 text-white';
    const style5 = 'hover:ease-in duration-300 border-2 mr-2 text-base font-black px-3 py-2 rounded-full bg-blue-900 text-white';
    ...
    const [slider, setSlider] = useState(Array.from(Array(3).keys(), x => x + 22));
    ...
    useEffect(() => {
        fetch("https://pacific-caverns-96128.herokuapp.com/https://www.ntv.com.tr/teknoloji.rss")
            .then((res) => res.text())
            ...
            if (feed) {
                for (let i = 0; i < 100; i++) {
                    if (i === 13 || i === 14 || i === 15 || i === 16 || i === 17 || i === 18 || i === 19){
                        if (feed.items[i].description.split('alt="')[1].split('" /><p>')[0].length > 40){
                            textContents[0].textContent.push(feed.items[i].description.split('alt="')[1].split('" /><p>')[0].slice(0, 40) + "...");
                        }
                    } else {
    ...
    <div>
        <div className='flex bg-black h-14 items-center text-2xl flex lg:pl-64 sm:pl-32'>
            <a href="https://ntv.com.tr" className=''>
                <img className='mr-36' src="https://cdn.ntv.com.tr/img/logo.svg?v=6429z" alt="ntv.com.tr"></img>
            </a>
        </div>
        <div>
                {
                    txt[0] ? (
                        <>
                            {
                                    <div>
                                        <p className='bg-blue-900 w-full text-white h-12 font-bold items-center text-2xl flex lg:pl-64 sm:pl-32'>Teknoloji Haberleri</p>
                                    </div>
                                }
                            </>
                        ) : null
                        ...
                        txt[0] ? (
                            <div className='w-full h-full relative bg-white flex flex-col justify-center'>
                                ...
                                <ul className='absolute bottom-0 flex justify-center flex-nowrap w-full cursor-pointer bg-white p-5'>
                                    ...
                                    <li onMouseEnter={() => numbers(2)} className={numberClass[1]}>2</li>
                                    <li onMouseEnter={() => numbers(3)} className={numberClass[2]}>3</li>
                                    <li onMouseEnter={() => numbers(4)} className={numberClass[3]}>4</li>
                                    <li onMouseEnter={() => numbers(5)} className={numberClass[4]}>5</li>
                                    <li onMouseEnter={() => numbers(6)} className={numberClass[5]}>6</li>
                                    <li onMouseEnter={() => numbers(7)} className={numberClass[6]}>7</li>
                                    <li onMouseEnter={() => numbers(8)} className={numberClass[7]}>8</li>
                                    <li onMouseEnter={() => numbers(9)} className={numberClass[8]}>9</li>
                                    <li onMouseEnter={() => numbers(10)} className={numberClass[9]}>10</li>
                                </ul>
                            </div>
                        )
                            : (<div>Loading...</div>)
                            ...
                            <>
                                {
                                    order1.map(item => (
                                        <div className='flex flex-col justify-center items-center col-span-3 md:col-span-1 row-span-1 w-full h-full bg-white grid grid-rows-3 sm:col-span-3 mx-auto'>
                                            ...
                                            <p className='row-span-1 my-auto px-2 text-black font-bold text-center'>{txt[0].textContent[item]}</p>
                                        </div>
                                    ))
                                }
                            </>
                            ...
                            <div className='col-span-6 row-span-1 h-3/4 mt-2 grid grid-cols-6 gap-4 bg-black p-4'>
                                {
                                    ...
                                    order2.map(item => (
                                        <div className='bg-sky-900 overflow-hidden bg-black hover:bg-slate-800 col-span-3 md:col-span-1 row-span-1 w-full h-full bg-white grid grid-rows-4 sm:col-span-3 mx-auto'>
                                            ...
                                            <p className='row-span-2 text-xs mt-2 px-2 text-white text-left'>{txt[0].textContent[item]}</p>
                                        </div>
                                    ))
                                }
                            </div>
